from django import forms

from django.contrib.auth.forms import UserCreationForm
from django.contrib.auth.models import User


class SignUpForm(UserCreationForm):
    first_name = forms.CharField(max_length=30, required=True, help_text='Your First name')
    last_name = forms.CharField(max_length=30, required=True, help_text='Your Last name')
    email = forms.EmailField(max_length=30, required=True, help_text='Your email address')
    birth_date = forms.DateField(help_text='required. Format YYYY-MM-DD')
    class Meta:
        model = User
        fields = ('first_name', 'last_name', 'email', 'username', 'password1', 'password2', 'birth_date')
